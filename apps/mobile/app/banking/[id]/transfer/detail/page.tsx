...
export default function TransferDetailPage() {
  ...
  const [targetAccount, setTargetAccount] = useState<{ targetName: string }>({ targetName: '' }); // 타겟 계좌 정보
  ...
  const handleSendMoney = async () => {
    ...
    if(accountInfo !== null && parseInt(amount) >= accountInfo!.balance){ // accountInfo가 null일 수도 있어서
    ...
    try{
      const response = await fetch(`http://localhost:8080/accounts/${myAccountId}/transfer`,{
        method:'POST',
        headers:{
